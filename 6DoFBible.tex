...
\begin{itemize}
    \item Improve parachute modeling
...
You can consider the door hinge as a mass-spring system constrained to motion along the $\hat{x}$-direction. The spring has an equilibrium length of $L_0$ and a spring constant of $k=5$ N/m with a mass of 1 kg. At time $t_0$, an instantaneous impulse of 10 $\mathrm{N}\cdot \mathrm{s}$ is applied to the hinge in the $-\hat{x}$-direction by a misguided ping-pong ball hitting the door. This is shown diagrammatically in \autoref{fig:mass spring}. Assume that the system is at rest in the equilibrium position at $t_0$.
\begin{figure}[ht]
    \centering
...
\Gls{oop}, like any skill in coding, is best understood with practice. As a starting point, we highly recommend \cite{tearle_object-oriented_nodate}, which gives a guided introduction to \gls{oop} in MATLAB.
...
Objects are inherently related to what are called \textit{classes}. A class defines the attributes and the functions that are associated with an object. The attributes associated with an object are called \textit{properties} and the associated functions are called \textit{methods}. Properties and methods are exactly analogous to the variables and functions seen elsewhere, but they are applicable only to objects of that class.
