...
In MATLAB, there exists a function - "angle2dcm" - that creates this for us!\footnote{Note there exists another similar function - eul2rotm - that also creates a DCM; however, this function converts a vector from the body frame to the inertial frame.} By inputting the scalar values of our \gls{Euler angles} (in radians!) as well as the chosen rotation sequence, MATLAB creates the DCM as an output, allowing us to pre-multiply this matrix with our inertial vector we want to express in the \gls{body frame}. Note the function allows an input of any number of rotations to find DCM's for. This is a fun little aspect of the function; however, it is irrelevant for our use as we input the new \gls{Euler angles} every time-step during our "ode45" integration scheme. One additional thing to consider: the "angle2dcm" function returns the DCM as a transform from the inertial to the \gls{body frame}. If we were to transform a vector from the body to \gls{inertial frame}, we would need to take the transpose of the given matrix. Further reading on this function can be seen in Mathwork's own documentation in Source \cite{mathworks_eul2quat_nodate}.
...
At this point, we would use our \gls{numerical int} scheme to approximate the next timestep's \gls{Euler angles} using the \gls{Euler rates} we just found. We won't do that here, but you can see a similar implementation in the 6DoF itself or the Dzhanibekov example!
...
\section{Practice Problems}
\begin{enumerate}
    \item You are flying home one day in your P-51 Mustang (pictured in \ref{} for your viewing pleasure). At cruise, your aircraft is 10,000 ft above sea level.
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{}
    \caption{Caption}
