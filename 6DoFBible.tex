...
\title{\Huge \textbf{Flight Dynamics Bible}  \\ \huge Volume I: \\6 Degree of Freedom Modeling}
% Author
...
\date{September 2025 \\ \newline \large{\textbf{Second Edition}}}
...
Some other areas are also covered in this document but are curtailed because Preston and I do not have the experience to give full authority on these subjects. However, as we learn more, these volumes continue to grow. In Volume II and III, we expand and add many of these other sections, especially those related to attitude dynamics, energy methods, and control theory, to better serve those in the future who may hope to recreate the work we have done here.
...
An ideal 6-\gls{dof} not only models the motion of a vehicle through the atmosphere as accurately as possible but is also well-structured and easily readable. Strict adherence to an agreed upon and appropriate set of coding standards is essential for a successful program. These standards cover everything from naming conventions to heading structure, and make the integration of codes from multiple contributors more seamless.
...
Our pedagogical approach is strongly reliant on examples and trying problems. We find that this is the best method for readers to learn the content. In each chapter (aside from this overview chapter), there will be some additional content at the end that may be especially useful for quick reference. For quick reference, we include a `Key Ideas' section that may be used to quickly reference the big takeaways from each chapter.
...
To aid in comprehensibility, we have added sources that we find especially useful and avoided the use of superfluous sources. These sources are attached in the bibliography for ease of access. In addition, useful links and footnotes have been highlighted to be easily located. 
...
The essence of the 6-\gls{dof} is to simulate the movements of a system through time. To do so, we need some description of how the system may respond to certain forces. We will spend quite a while discussing how these forces arise and how we may use them to understand the subsequent motion of a vehicle. However, as many of you likely remember from your physics classes, forces acting on a rigid body or more generally about a point give rise to moments. These moments impart some angular motion on our system, which motivates the study of mathematically describing orientation due to these rotations. After understanding how to describe both the translational and rotational motion of a rigid body, the dependence of aspects of these equations on variables we must integrate to solve form. This conundrum is mostly avoided through numerical integration, which allows us to approximate the motion of our vehicle governed by these derived equations. Before this; however, we must also consider how to accurately model the aerodynamic forces such as lift and drag on our vehicle. While equations are known, certain parameters within these equations are themselves functions of multiple other parameters, meaning computer simulations are required to accurately determine them. Finally, at the very end, we will be discussing the 6-DoF itself and the coding practices used in its construction. To help you better understand the framework, we have included a diagram below for a visualization.




Talk about forces into moments, rotations, and why numerical integration is required. Mention aerodynamic forces. Include diagram similar to the one shown in the code review.
...
Our definition strays from particle dynamics because we are considering a rigid body which can exhibit rotation. The exploration of this rotation will be explored in much more detail in \autoref{sec:energy methods}, \autoref{sec:attitude dynamics}, and Volume II and III. Here, we will introduce the methods used in our future analysis.
...
A total of three forces are shown, all of which act along the $\hat{y}$-direction. Because all of the forces lie along the same direction, they are collinear, and the dynamics can be described with only one axis. This feature is what makes this a 1-\gls{dof} model. The three forces shown are the thrust force, the drag force, and the gravity force.

Following what you may do in an introductory physics class, we will describe each of these forces via their functional form. These are described in \eqautoref{force table}:
...
It is helpful to think about these quantities as vectors, because we can add them together to achieve a resultant.\endnote{More formally, we define these vectors as existing a linear space, where multiplicative and additive properties are preserved. This formal definition allows us to apply this to any arbitrarily number of dimensions. For our uses, this will usually be up to 3 dimensions.} In the 1-dimensional case this is quite trivial since all vectors lie along the $\hat{y}$ unit vector, but this will become increasingly important as we move to higher dimensions.

\subsection{Numerical Integration in the 1-DoF}\label{sec: Numerical Integration in the 1DoF}

Now we will move into something that you not have seen before. Given all the forces, we apply Newton’s 2\textsuperscript{nd} Law, $\sum{\vec{F}}=m\vec{a}$ , to arrive at an expression for the acceleration, known as an \gls{eom}, as shown in \eqref{eq2}. Doing so might look something like this:
\begin{gather}
...
What we have done is formally called writing a \gls{difference equation}.  Now only is this important for numerical integration, but in digital processing, because computers must discretize a signal! We will see these equation again when we study practical control systems in Volume III.
...
We remember that $F_D$ is a function of $v$. We will show this by writing $F_D=\kappa v^2$. The variable $\kappa$ is used since all other parts of the drag are independent of velocity. Since we don't know $v_{new}$, we have to use $v_{old}$ as the value of $v$ in the drag equation.

So, we can do a rearrangement of the equation to arrive at the final form:
$$v_{new}=\left(\frac{F_T-\kappa {v_{old}}^2}{m}-g\right)\Delta t+v_{old}$$
Lastly, we define $v_0$ to be 0 for most cases. The iteration through values of $\Delta t$ is the exact same as before. As we can see, the only real difference that is incurred is that we use the value of $v_{old}$ in our drag expression! Otherwise, this integration looks nearly identical to the example with velocity.
\end{enumerate}

We should note that this method of numerical integration is not used too often in practical applications. However, writing difference equations \textit{is} quite important, so it's still a good skill to have nonetheless.

In future applications of numerical integration, the most important skill to have is to be able to construct the \gls{eom} and to seperate the equation into first order equations.
\end{example}

If you still feel slightly unsure about some details, note that a more formal definition of Euler’s method is given in \autoref{sec:euler integration}. This may help clear up any confusion here.

We also show this Euler Integrator example in MATLAB \autoref{Script 1} below. Note that $\frac{1}{2}\rho_{\infty}SC_D$ is called $k$ in the script for simplicity. This simplification assumes that $\rho_{\infty}$ and the $C_D$ are constant, which we will later see is a quite poor assumption for rocket modeling but is okay for a first approximation.

\newpage
\lstinputlisting[style=Matlab-editor,  caption=1 DoF]{6DoF Explanation Scripts/OneDofEuler.m}\label{Script 1}
...
Relating this to the \gls{body frame}, this translates to an offset from the \gls{body frame} by the \gls{angle of attack} and sideslip. This frame is often useful for aircraft, but we do not use it explicitly in our 6-\gls{dof}. The \gls{wind frame} itself is in fact skipped over during wind calculations, as the \gls{angle of attack} and imparted forces/moments are calculated directly onto the \gls{body frame}. So as this isn’t as useful as the other two reference frames, we won’t elaborate on this frame any further, and it won’t be shown as much in our example 6-\gls{dof}.

\section{Derivative Transport}
The last issue that we tackle when different reference frames get involved is the fact that objects are moving around, specifically accelerating, so it can be hard to express quantities with just an inertial frame. We can create various reference frames, but to correctly apply Newton's second law, we must use our inertial frame as the original reference.
...
\begin{example}[The Frisbee]
    ...
    We will use some of our tools with reference frames to help us. We will establish an inertially fixed frame, $\hat{e}$. This frame will stand still and gives us a `ruler' to measure everything against. 
    
    We also want a reference frame that is embedded in the frisbee, which we may call $\hat{a}$. We can embed the $\hat{a}$-frame in the Frisbee such that $\hat{e}_3$ and $\hat{a}_3$ are both pointing out of the page. We can construct a visual of this, as shown in \autoref{fig:frisbee}. In this figure, we can see that the Frisbee has some rotation with respect to the $\hat{e}$-frame.
...
Our end objective is to describe the position, velocity, and acceleration of this point $P$ on the Frisbee. We may start by describing the position, which is the easiest to handle. Say we want to describe the location of point $P$ on the Frisbee. We can do this by expressing the location of point $A$ from the origin in the $\hat{e}$-frame, and then go the rest of the way in the $\hat{a}$-frame. That is to say, we can describe the position of the center of mass ($A$) and then it is simply a translation along the $\hat{a}_1$-direction to arrive at point $P$. Doing so would look like:
$$\vec{r}^{\ OP}=\vec{r}^{\ OA} + \vec{r}^{\ AP}$$
Let us attach some variables to this to make it more tangible. Say we have thrown the pendulum directly north, along the $\hat{e}_2$ direction, by $30 \mathrm{\ m}$. This gives us $\vec{r}^{\ OA}$, which is $30 \hat{e}_2$. $\vec{r}^{\ AP}$ is computed similarly. Taking $R$ as the radius, this vector is $R\hat{a}_1$. Altogether:
$$\vec{r}^{\ OP}=30\hat{e}_2 + R\hat{a}_1$$

Some complication comes when we try to express the velocity of point $A$. We know that the velocity is the time derivative of the position, but our position is being described using non-inertial coordinates in the $\hat{a}$ frame!

Using our standard derivative, there is no machinery that allows us to take this derivative in the $\hat{a}$-frame. We can solve this by computing the rotation of one reference frame with respect to the other. This will give us the ability to compute a valid derivative.

Going back to our frisbee, we can notice that $\hat{a}$ and $\hat{e}$ are offset by some angle. If we know this angle, and the rate at which it is changing, we can understand the relationship between the $\hat{a}$ and the $\hat{e}$ frame.
\begin{figure}[H]
    \centering
...
\end{figure}
So we have an angle between the two frames, $\theta$, and an angular rate, $\omega$. We want to find an expression for the infinitesimal change in $\hat{a}$. 
\begin{figure}[H]
    \centering
...
\end{tikzpicture}
    ...
    \label{fig:inf rot}
\end{figure}
Considering an infinitesimal rotation, $r$ will be displaced in a direction that is orthogonal to its current direction due to the rotation by $\omega$. We can see that this $\dot{r}$ is orthogonal to both $r$ and $\omega$ (which is pointing out of the page). Following the \gls{rhr}, this direction must be given by $\omega\times\vec{r}$.

This cross product also gives us an appropriate magnitude for the vector, as the magnitude will increase with a larger $\omega$ or if the length of the lever arm, $r$, increases.

There are more fundamental reasons for this cross-product relationship, but these will have to wait until Volume II and III to be explained by Lie theory.

With the rotation considered, the rest is quite simple. We must consider the change of $r$ itself, so we consider the derivative of $r$ with respect to the $\hat{a}$-frame to do so. Altogether, we have the full \gls{bke}:
\begin{gather}
    \color{blue}\boxed{
    \color{black}{}^e\frac{d}{dt}\vec{r}={}^a\frac{d}{dt}{\vec{r}}+{}^e\vec{\omega}^a\times\vec{r}}\\
    \color{blue}\mathrm{Basic \ Kinematic\ Equation}
\end{gather}

\end{example}

In general, we can use the \gls{bke} to express how frame translations affect the derivatives of states. We can perform analysis using multiple intermediate frames to build a sequence of rotations, or apply the \gls{bke} twice to get an acceleration.
...
\begin{gather}
    ...
    \end{bmatrix}
\end{gather}
This is done not only for the sake of compactness, but also to facilitate the use of matrix operations for translation between reference frames. This concept is further explored in \autoref{sec:Frame of Reference}. For this reason, expressing these elements in vector form will become crucial as we move forward. In MATLAB, we express a tri-dimensional column vector as:
...
We present below a modified version of the 1-\gls{dof} below in full 3D in \autoref{fig:3 DoF Forces}.
...
We will show how to compute this moment if the location of the center of gravity and the center of pressure is known. To show this, we will briefly return to the 2D case for illustration, but keep in mind that this concept is extensible in 3D. We show this in \autoref{fig:Moments}.
...
\draw   (464,61) .. controls (464,55.48) and (468.48,51) .. (474,51) .. controls (479.52,51) and (484,55.48) .. (484,61) .. controls (484,66.52) and (479.52,71) .. (474,71) .. controls (468.48,71) and (464,66.52) .. (464,61) -- cycle ;
%Shape: Pie [id:dp22926947041960688] 
\draw  [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ,fill opacity=1 ] (474,71) .. controls (474,71) and (474,71) .. (474,71) .. controls (474,71) and (474,71) .. (474,71) .. controls (468.48,71) and (464,66.52) .. (464,61) -- (474,61) -- cycle ;
%Shape: Pie [id:dp053159935463349295] 
\draw  [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ,fill opacity=1 ] (474,51) .. controls (474,51) and (474,51) .. (474,51) .. controls (479.52,51) and (484,55.48) .. (484,61) -- (474,61) -- cycle ;
%Shape: Circle [id:dp1802989144310676] 
\draw  [color={rgb, 255:red, 74; green, 144; blue, 226 }  ,draw opacity=1 ][fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=1 ] (464,101) .. controls (464,95.48) and (468.48,91) .. (474,91) .. controls (479.52,91) and (484,95.48) .. (484,101) .. controls (484,106.52) and (479.52,111) .. (474,111) .. controls (468.48,111) and (464,106.52) .. (464,101) -- cycle ;
%Straight Lines [id:da11363929006275086] 
\draw    (90,170) -- (90,132) ;
\draw [shift={(90,130)}, rotate = 90] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (7.65,-2.3) .. controls (4.86,-0.97) and (2.31,-0.21) .. (0,0) .. controls (2.31,0.21) and (4.86,0.98) .. (7.65,2.3)   ;
%Straight Lines [id:da09061120859780936] 
\draw    (90,170) -- (124,170) ;
\draw [shift={(126,170)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (7.65,-2.3) .. controls (4.86,-0.97) and (2.31,-0.21) .. (0,0) .. controls (2.31,0.21) and (4.86,0.98) .. (7.65,2.3)   ;
%Shape: Arc [id:dp7211379182460548] 
\draw  [draw opacity=0] (90,180) .. controls (90,180) and (90,180) .. (90,180) .. controls (90,180) and (90,180) .. (90,180) .. controls (84.48,180) and (80,175.52) .. (80,170) .. controls (80,164.48) and (84.48,160) .. (90,160) .. controls (95.52,160) and (100,164.48) .. (100,170) -- (90,170) -- cycle ; \draw   (90,180) .. controls (90,180) and (90,180) .. (90,180) .. controls (90,180) and (90,180) .. (90,180) .. controls (84.48,180) and (80,175.52) .. (80,170) .. controls (80,164.48) and (84.48,160) .. (90,160) .. controls (95.52,160) and (100,164.48) .. (100,170) ;  
\draw   (85,175) .. controls (86.67,177.78) and (88.33,179.44) .. (90,180) .. controls (88.33,180.56) and (86.67,182.22) .. (85,185) ;
...
\draw  [draw opacity=0] (483.67,189.66) .. controls (482.48,189.88) and (481.25,190) .. (480,190) .. controls (468.95,190) and (460,181.05) .. (460,170) .. controls (460,158.95) and (468.95,150) .. (480,150) .. controls (491.05,150) and (500,158.95) .. (500,170) .. controls (500,170.15) and (500,170.29) .. (500,170.44) -- (480,170) -- cycle ; \draw    (483.67,189.66) .. controls (482.48,189.88) and (481.25,190) .. (480,190) .. controls (468.95,190) and (460,181.05) .. (460,170) .. controls (460,158.95) and (468.95,150) .. (480,150) .. controls (491.05,150) and (500,158.95) .. (500,170) ; \draw [shift={(500,170.44)}, rotate = 259.81] [fill={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.08]  [draw opacity=0] (8.93,-4.29) -- (0,0) -- (8.93,4.29) -- cycle    ; 
...
% Text Node
\draw (321,114.4) node [anchor=north west][inner sep=0.75pt]    {$\stackrel {\rightharpoonup}{r}_{ma}$};
% Text Node
\draw (493,53.4) node [anchor=north west][inner sep=0.75pt]    {$\stackrel {\rightharpoonup}{r}_{cg}$};
% Text Node
\draw (495,91.4) node [anchor=north west][inner sep=0.75pt]    {$\stackrel {\rightharpoonup}{r}_{cp}$};
% Text Node
\draw (127,160.4) node [anchor=north west][inner sep=0.75pt]    {$\hat{x}$};
% Text Node
\draw (93,112.4) node [anchor=north west][inner sep=0.75pt]    {$\hat{y}$};
% Text Node
\draw (92,183.4) node [anchor=north west][inner sep=0.75pt]    {$\hat{z}$};
% Text Node
\draw (491,182.4) node [anchor=north west][inner sep=0.75pt]    {$M_{Aero}$};
...
One thing to note is that some quantities are more easily defined with respect to vectors that are defined with respect to the vehicle (which we will call the \gls{body frame}), such as the vector $\hat{X}$. As seen in Equation \eqref{eq:alpha} and \eqref{eq:Lift}, we often want to use these vectors in the \gls{body frame} for computation. As such, we will need a method in which we can convert between the body and \glspl{inertial frame}. This conversion between frames is explored in \autoref{sec:Frame of Reference}. We should also note that our moments will be in this \gls{body frame} as well, which turns out to be especially useful in \autoref{Euler Rotation Equations}.
...
As a last note for this section, we will briefly discuss the concept of a \gls{state vector}. The \gls{state vector} is a column vector that contains information on the current state of our system. In the example of our 3-\gls{dof}, we will include the position, velocity, rotation, and rotation rate of the system in the \gls{state vector}. This would look like\endnote{The order of the state vector does not particularly matter, so long as we are consistent with the location of the elements. Typically, it is common to write all vectors first and then their derivatives, but there is no hard rule. We follow this convention in our discussion.}:
$$\vec{X}=\begin{bmatrix}
...
In \autoref{sec:moments} we have explored the moments that are created when a force acts at a point that is not through the center of mass. Here, we will make the jump from particle dynamics into \gls{rigid body} dynamics.
...
\begin{equation}\label{eq:IwVec}
    \vec{H}=I\vec{\omega}
\end{equation}
In the simplification using the \gls{principal axes}, we often refer to the terms $I_{xx}$, $I_{yy}$, and $I_{zz}$ simply as $I_x$, $I_y$, and $I_z$. There are called the \glspl{pmoi}.
...
$$\begin{array}{l}
    ...
    {z}=r\sin\theta
\end{array}$$
We can use these relations in our moment of inertia integrals \eqref{eq:moi}. We'll start by computing the moment in the $x$-direction, substituting in our polar coordinates:
\begin{gather}
    I_{xx}=\iiint\limits_V{\rho}{\left(y^2+z^2\right)}{dV}\rightarrow I_{xx}=\iiint\limits_V{\rho}{\left[(r\cos\theta)^2+(r\sin\theta)^2\right]}{dV}
\end{gather}
...
\begin{itemize}
    ...
    \item The $\Delta V$ between the launch vehicle and the satellite after the burn, assuming the total impulse requirement is met.
...
Since $\Delta t$ is unknown, we consider the left-hand side of the equality. Taking $\Delta \omega_x=\omega_f-\omega_i=2\pi-0$ and $I_{xx}$ from before, we have a total impulse requirement of $0.3217 \ \mathrm{N{\cdot} m{\cdot}s}$.

\subsubsection{Despin}
...
Energy methods of mechanics employ the concepts of the energy of a system to solve a problem in mechanics. This is in contrast to the Newtonian method, which considers the forces of a system. The good news is that both will result in the same answer (see proof in \autoref{equiv newt and lang}). However, having both in your set of tools as an engineer can prove especially useful. To grasp energy methods, it is best to have an intuition for Newtonian dynamics first. A thorough reading of \autoref{sec: Newtonian Dynamics} and other sources will help prepare the reader for this section.

There are two main types of energy mechanics, referred to as Hamiltonian and \gls{Lagrangian} mechanics. In this chapter, we will focus on the \gls{Lagrangian} version of these mechanics. Hamiltonian mechanics are arguably more fundamental, but we will leave that discussion for Volume III.

The energy methods of mechanics are more advanced than the other topics discussed here. These topics will require a bit of background in multivariate calculus and are not strictly required for understanding the rest of the 6-\gls{dof} implementation. However, we believe it is best to be exposed to these ideas early such that the reader can gain an appreciation for where more advanced mechanics (and the subsequent volumes of this text) may take them.

Although the Hamiltonian and \gls{Lagrangian} are not directly used in the case of our 6-\gls{dof}, they are fundamental to other areas of mechanics and make derivations simpler in many cases. The Hamiltonian and \gls{Lagrangian} descriptions of mechanics are more often taught to physics students rather than engineers, but we feel that they deserve inclusion among the topics presented here. You may find that certain \gls{eom} derivations, such as the spinning top and problems in orbital mechanics are more easily accomplished with energy methods.

Another reason to motivate these systems is that these energy methods are rotationally invariant because energy methods can employ a generalized coordinate system. These generalized coordinates are very powerful and allow us to describe a huge variety of systems. We will see this later in some examples. A brief overview of the differences between the two schemes of \gls{eom} derivation is shown in \autoref{fig:newt v lang}.
...
\end{figure}
The discussion of these systems in the volume is curtailed because it is not directly applicable to the work on our specific rocket 6-\gls{dof} model. However, the fundamental implications of energy methods are important, but that full discussion is saved for Volume III. A mathematical background on the calculus of variations, which provides a derivation for Lagrangian mechanics, is also presented in Volume III. We present here only the formulaic approach and its used.

\section{Hamilton's Principle}

Hamilton's Principle is the fact that underlies all of energy mechanics. To understand energy mechanics at all, we must study this idea closely. Before we can can describe Hamilton's principle, we need to understand a new type of calculus, one which is the calculus of whole functions. We call this type of calculus the `calculus of variations'. The calculus of variations underpins the ideas of \gls{Lagrangian} and Hamiltonian mechanics, so it is important to understand the principles behind it before moving on to the actual mechanics. In analogy to how single variable calculus is fundamental to Newtonian mechanics, the calculus of variations is fundamental to energy mechanics.

The fundamental idea of the calculus of variations is simple. We want to find the maximums and minimums of some quantity in our problem. Why? In physics, we often see that quantities are minimized or maximized for some reason. For example, a soap surface will minimize its surface area, or a beam of photons will follow a \gls{geodesic} of spacetime. Light will also refract through surfaces to minimize time as seen in Snell's Law.\endnote{More information on all of these topics is found in the introductory part of Volume III.}

Because so many physical phenomena are governed by this behavior to minimize certain quantities, Hamilton (the same mathematical hero we will see with attitude dynamics), decided to formulate a mathematical description of this.

In essence, our goal is to find a function that is the minimum of some set of conditions. These conditions also take the form of a function, so we attempt to find a function that is a minimum of another function. We call this quantity which we are minimizing a \gls{functional}, which we define as:
\begin{equation}\label{eq:functional}
    J=\int_{x_1}^{x_2}f\left[y(x),y'(x);x\right]{dx}
\end{equation}
The \gls{functional}, $J$, is a quantity which maps a function onto the real number line. This means that it takes the function as an input and outputs a single real number (that is, a function of a function). We are attempting to find an input function, $y(x)$, that yields an extrema of $J$. Our goal is to find criteria for $y(x)$ that makes $J$ a maximum or a minimum.

For now, we present the way to achieve this without derivation. The fundamental equation of the calculus of variations is called the \gls{Euler Equation}:
\begin{equation}\label{eq: Euler's Diff eq}
    \frac{\partial f}{\partial y}-\frac{d}{dx}\frac{\partial f}{\partial y'}=0
\end{equation}
Really, what we have done is find a condition such that the integrand is zero. We define a quantity $\delta$, the variation, which can help us show this. We can reformulate the problem of calculus of variations to the following:
\begin{equation}
...
In \autoref{Euler Rotation Equations}, we explored the Newtonian method of deriving the Euler rotation equations using the \gls{bke}. Here, we present an alternative derivation that utilizes the \gls{Lagrangian} approach and is ultimately a more fundamental derivation.
...
\end{equation}
We can select our generalized coordinates as the three Euler angles, $\psi$, $\theta$, and $\phi$ (see \autoref{sec:Euler Angles}).
...
\item  Jonah wakes up one day and he is running late to his dynamics final! In a rush, he hops on his Onewheel and starts riding to class. You can model Jonah as a mass on the end of a massless rod and his Onewheel as a cart constrained to motion along the $\hat{x}$-axis. The angle of Jonah from the vertical is denoted as $\theta$. Derive the \gls{eom} of the system using energy methods. The set-up is shown in \autoref{fig:Inverted Pendulum}.
\begin{figure}[ht]
    \centering
...
As with our translational values, we want to take the integral of our rate of change of these angular parameters in order to find our new, updated Euler angles. While it might be easy to assume the values we would be integrating are the angular velocities, doing so DOES NOT return the \gls{Euler angles}. Giving a quick reason as to why: angular velocity is expressed exclusively with respect to the body frame, while our Euler angles are defined about their corresponding intermediate frame in the rotation sequence. Meaning this job for integration is left to \gls{Euler rates}, something else we need to include in the code. There exists what we affectionately call the ``B'' matrix that allows us to use \gls{Euler angles} to express a relation between \gls{Euler rates} and angular velocities. Unfortunately, there doesn't exist a MATLAB function that can calculate the ``B'' matrix, leaving us to our own devices to construct one from scratch. The heavy lifting for this is completed in Volume II, meaning we can copy the matrix derived there straight into our MATLAB script. That matrix and equation is as follows, for quick reference:
\begin{gather}
...
\lstinputlisting[style = matlab-editor, language=Matlab, caption=Euler Angle Dynamics Example]{6DoF Explanation Scripts/Attitude Dynamics Examples/GravityToBodyEuler.m}\label{Euler Dynamics Listing}
...
To match industry/academia standards and improve the robustness of our simulation, our 6DoF model uses \glspl{quaternion}. The use of normalized hyper-complex numbers allows us to avoid losing a degree of freedom at a specific orientation of our rocket (called gimbal lock), which is what makes the simulation more robust. Just as with \gls{Euler angles}, MATLAB has an extensive suite of functions that makes the implementation of \glspl{quaternion} just as easy. 

\subsection{Quaternion Rotation Matrix}

We are going to combine a few initial steps of the code surrounding \gls{quaternion} based attitude dynamics. To start, we want to convert from \gls{Euler angles} to \glspl{quaternion}, as \gls{Euler angles} are more intuitive to define and initialize in the simulation. MATLAB's function "eul2quat" does just that. By inputting the \gls{Euler angles} in the appropriate sequence, along with the chosen rotation sequence (make sure to match the \gls{Euler angles} with the order), the function returns the corresponding \gls{quaternion}. Mathworks documentation for this function can be found at Source \cite{mathworks_eul2quat_nodate}.

Having initialized our \gls{quaternion}, we can now use another MATLAB function to construct our DCM. The function "quat2dcm" takes the \gls{quaternion} as an input, and returns the DCM at that instant.\footnote{This step begins the divergence from the true method of quaternion encoded rotations, as we choose to use elements of the quaternion to build the elements of the DCM for clarity's sake. True quaternion encoded rotations are discussed in-depth in Volume II.} Incredibly simple and incredibly useful; however, we must once again consider that this DCM converts only from the \gls{inertial frame} to the \gls{body frame}. Meaning just as we had done with \gls{Euler angles}, it is necessary to take the transpose (inverse) of the matrix if we want to transform from the \gls{body frame} to the \gls{inertial frame}. The Mathworks documentation for the "quat2dcm" function can be found at Source \cite{mathworks_quat2dcm_nodate}.

\subsection{Quaternion Rates}\label{sec: quaternion rates}

Just as we had done for \gls{Euler angles} with \gls{Euler rates}, we want to find the \gls{quaternion} rates that allows us to integrate and obtain a new \gls{quaternion}. Also just as with \gls{Euler angles}, there is no MATLAB function allowing us to relate \gls{quaternion} rates with angular velocity. But once again, just as with \gls{Euler angles}, we can derive the necessary method to do so! The outcome of this derivation is shown in \eqref{eq: quat rates}. So plugging it into our script is an easy fix to make this operation possible. The calculation is included here for quick reference:
...
Our initialization of the \gls{state vector} and all of the basic setup in contained in the main file, shown in "DzhanibekovMain.m". The "DzhanibekovIntegrator.m" code operates on the state vector using "ode45" to get the derivative. This code is . There is additionally a file to display the rotation of the body in an animation, which is "RotationVisualizer.m". This code can be found in \autoref{sec:Dzanibekov Code}.
...
\begin{enumerate}
    \item Your friend Jimothy, impressed by your MATLAB skills, has challenged you to model a spinning top. The top is axially symmetric about its $\hat{X}$-axis and has an initial spin rate of 1200 rpm and has an initial angle to the vertical of $20\degree$. The top spins down at a rate governed by an exponential decay of form $\omega(t)=\omega_0e^{\frac{1}{10}\ln(2)t}$ such that the spin rate is halved every 10 seconds. Gravity acts on the top in the negative $\hat{x}$-direction. The top has a mass of 20 grams. Relevant details are given in \autoref{fig:Spinning Top}.
...
Aerodynamics is a topic of great concern to us, as drag is one of the primary forces experienced by our rocket. In \autoref{sec: 3DoF Case} we discussed the 4 fundamental forces on a vehicle and how we define their directions. In that section we did not define the magnitude of the aerodynamic forces, $\vec{F}_L$ and $\vec{F}_D$. The goal of aerodynamics, put shortly, is to calculate these values to the best of our ability.

There is a large amount of analysis that can be done in order to understand the lift and drag that act on our vehicle. These aerodynamic phenomena -- especially as we look into modeling transonic and supersonic rocket systems -- can be very complicated. In order to keep this section relatively comprehensible, we have decided to only cover a few basic concepts. Some fairly basic modeling can get us 80\% of the way to good aerodynamics. The last 20\% requires very complex modeling which are beyond the scope of this first volume.

We may start our discussion by introducing one of the most fundamental ideas in fluid dynamics: non-dimensionalization. Aerodynamics very often uses non-dimensional quantities, so we may start our exploration of aerodynamics here.

\section{Non-Dimensionalization}

If you have studied any aerodynamics, you have likely seen the equations for lift or drag:
$$\frac{1}{2}\rho_{\infty}V^2_{\infty}SC_X$$
This coefficient, $C_X$, is a non-dimensional quantity. It describes the force generated via lift or drag, but the value itself has no dimension. A large portion of aerodynamics is devoted to figuring out how we can calculate these non-dimensional values!

Before we start with calculations of these values, we should  understanding how non-dimensionalized quantities emerge, and which ones are important for aerodynamics. This will help us understand why we have many equations with the term $\frac{1}{2}\rho V^2$, or why the mach number is important. Along the way, we will also discover some other important properties that influence the airflow. 

Since our main goal is to understand the lift and the drag on an object, we will start by enumerating the various factors that could impact the aerodynamic forces experienced by an object. Some intuition will tell us these forces are likely affected by the following factors:
\begin{enumerate}
...
\begin{example}[Aerodynamic Forces]
    Using the system characteristics described before, let us use \gls{buck pi} to write the non-dimensional groups for our aerodynamic system. We start by writing the dimensions for each of the variables we enumerated:
...
Applying \gls{buck pi}, we can create 4 $\Pi$ groups from the 7 variables. The solving of these $\Pi$ groups is shown in the Appendix in \autoref{Buckingham Pi Deriv} because it is quite lengthy.

Solving the $\Pi$ groups, we get:
\begin{gather}
    \Pi_1=\frac{\rho VC}{\mu}\\
    \Pi_2=\frac{V}{a}\\
    \Pi_3=\frac{F}{\rho V^2C}\\
    \Pi_4=\alpha
\end{gather}
...
From \autoref{thm: buck pi corollary}, we can also say that the force coefficient must be a function of the other $\Pi$ groups:
$$\color{blue}\boxed{\color{black}C_x=f\left(Re,M,\alpha\right)}$$
\end{example}

The conclusion drawn from the \gls{buck pi} is that we can determine the force coefficients using only the $Re$, $M$, and $\alpha$. Thus, whenever we do modeling of aerodynamics, we only need to consider Reynolds number, Mach number, and \gls{angle of attack}. This is a cornerstone of how we model aerodynamics in our 6-\gls{dof} modeling.

\section{Lift and Drag}
Determining the lift and drag of the our rocket is the main purpose of aerodynamics for our applications. 

\section{Aerodynamics Terminology}
Aerodynamics has a lot of terminology associated with it that is important to understand before we can approach modeling the aerodynamics of the system. We will mostly cover aerodynamics terminology for rockets, but the modeling done here should be transferable to aspects of aircraft systems as well.\endnote{The principles we describe are the same, but much of the modelling is simpler for rockets since they have axial symmetry in most cases. We will utilize these assumptions early on to simplify this section.}
...
The normal force acts normal in the $\hat{y}$ direction and the axial force acts in the $\hat {x}$ direction as shown in \autoref{fig:pressure and shear}. Normal and axial forces are not commonly used in our modeling outside of these calculations. More often, we see the quantities lift and drag, which are defined with respect to the \gls{freestream} velocity. We can easily transform our normal and axial forces into a lift and drag using a rotation matrix with our \gls{angle of attack}, $\alpha$. This rotation matrix takes the form:
...
\end{gather}
We note that the standard convention in aerodynamics for the moment about the nose is for a positive moment to denote a pitch up. This is opposite the convention established by the \gls{rhr}. For this reason, we choose to use a more standard sign convention in this equation. For more information on the derivation of the equations in this section, refer to Chapters 1 and 2 of \cite{anderson_fundamentals_2017}.
...
Getting live weather information is useful in improving upon the uncertainty in the calculations provided by the "atmosisa" model. To do so, our model uses information from the API here \cite{openmeteo_openmeteo_nodate}. Another benefit of such live weather data is that it allows us to compute wind at the surface level and at many pressure levels (up to $22 \mathrm{\ km}$ in height). 
...
Sometimes, we can only get weather data for close to the surface from real time or historic measurements. In this case, it is often a good approximation to use the standard atmosphere model provided by something like "atmosisa" offset to match the temperature and pressure measured.
...
\section{Practice Problems}
\begin{enumerate}
    \item You are flying home one day in your P-51 Mustang (pictured in \autoref{fig:p51} for your viewing pleasure). At cruise, your aircraft is 10,000 ft above sea level.
\begin{figure}[ht]
    \centering
...
Our RK2 algorithm is a direct translation of \eqref{eq:rk2} into code. We present this below in \autoref{code:rk2}:
...
\end{figure}
The code that runs this example is shown in \autoref{sec:numerical int error code} in the Appendix. This code is a good example for understanding the syntax for using the integration code snippets in Listing \autoref{code:rk2} and \autoref{code:rk4}.
...
We previously discuss the concept of \gls{stiff} systems in \autoref{sampling and stiffness}. 
...
MATLAB has no built in functions for variational or \glspl{symplectic int}, so we do not discuss them further here for now. Further discussion is in Volume III.
...
\chapter{The 6-DoF}\label{sec:6DoF}
...
Our definitions of our frames follow from those discussed in \autoref{sec:Frame of Reference}. For the earth-centered \gls{inertial frame}, we follow the conventions discussed in \autoref{fig:InertialFrame}. Recall that the $\hat{x}$ axis points upward toward the zenith, the $\hat{y}$ axis points east along the surface, and the $\hat{z}$ axis finishes the right-handed coordinate system, pointing north.
...
Here, we introduce the use of the rotation matrix to convert between different frames. MATLAB allows for the creation of a rotation matrix with the input of a \gls{quaternion} using the "quat2dcm" function. Using this, we can perform frame conversion to and from the body and earth frame. We show this below in \autoref{code:RotationMatrix}:
...
Our other two forces follow in a similar manner. The quantities of lift and drag are heavily discussed in \autoref{sec: aerodynamics}, so we will keep them succinct here. The directions of lift and drag in the earth frame are described by \eqref{eq:Lift} and \eqref{eq:drag}. To convert these forces into the \gls{body frame}, we use the function call "RotationMatrix(force, quat, 0)". The specification of "0" in the function converts a force in the earth frame into one in the \gls{body frame}. We need the \gls{body frame} force for computing the moments in the \gls{body frame}, as discussed in \autoref{sec:rotations in the body frame}.
...
In addition, it is necessary to consider the time delay of inflation, as the parachute will exponentially inflate as it is deployed. This means the equation for the area in our drag calculation is the following: $S=0.7\pi (\frac{1}{2}D)^2(1-e^{-.5t_{Drogue}})$. After the drag calculation, it is applied to the opposite of the freestream velocity. This force is only active when the vertical velocity is negative, so this is controlled by an "if/else" structure as: "if vel(1) < 0". In addition, our vehicle has both a drogue and main chute. As such, another if/else structure controls the deployment of the main chute at a specified altitude. The parachute dynamics are far more complicated than this, but currently we do not model this added complexity. 
...
Lastly, the \gls{quaternion} rates are described in the same as in \autoref{sec:Dzhan}.
\begin{equation}
...
\chapter{MATLAB and Coding Practices}\label{sec:MATLAB Practices}
...
That being said, as our system grows in complexity, following these practices becomes increasingly important for organization. If the reader is working on their own large code base or contribute to our code base, it is a good idea to follow these practices as well to stay organized.

\section{Object Oriented Programming in MATLAB}

\color{black}

Up until now, we have mainly been describing a procedural approach to programming. The procedural approach uses multiple interacting functions to run the code. Each function takes in inputs and outputs. Together, these functions work together to run the code and built the final output. Each function is distinct from one another, and operate with only the information that we choose to pass into the function. In most introductory coding classes, this is the type of programming we learn.
...
\begin{figure}[ht]
    \centering
\begin{lstlisting}[style=Matlab-editor, caption=Procedural Programming Ball Example]
% attributes of a ball
radius = 0.1; %[m]
restitutionCoeff = 0.8;
color = [0,0,1];
\end{lstlisting}
    \label{fig:ball procedural}
\end{figure}

If we only simulate one ball, this is of course no hassle. There is no reason to complicate things more than this. However, if we want to simulate multiple balls and track each parameter separately, we can see how this would quickly get out of hand. A much better approach would have these properties by intrinsic to each ball.

To accomplish this, we will now discuss how we can programmatically create these objects and construct whichever features we desire with classes.

\subsubsection{Classes}

Objects are inherently related to what are called \textit{classes}. A class defines the attributes and the functions that are associated with an object. The class is essentially a blueprint for what an object may look like.

The attributes associated with an object are called \textit{properties} and the associated functions are called \textit{methods}. Properties and methods are exactly analogous to the variables and functions seen elsewhere, but they are applicable only to objects of that class.

In MATLAB, these classes can be created by clicking the dropdown when creating a new file and selecting `Class'. The default class looks like:

\begin{figure}[ht]
    \centering
\begin{lstlisting}[style=Matlab-editor, caption=Class Example]
classdef untitled
...
\begin{figure}[ht]
    \centering
\begin{lstlisting}[style=Matlab-editor, caption=Class Constructor Example]
    properties
        Name (1,1) string
...
This can be accomplished through the use of a program such as GitHub Actions, which checks code each time it is pushed to a new branch. We have set up a continuous integration platform for our 6-\gls{dof} already, but more information about setting up such a system can be found \cite{mathworks_continuous_2025}.
...
The code in \autoref{code:startup} is the default plotting used in this document, which should be a good baseline for professional plots.
...
\subsubsection{C \& D}
\newpage
\begin{lstlisting}[style=Matlab-editor, caption=Mass Spring Damper Solution]
% PSP FLIGHT DYNAMICS:
...
\lstinputlisting[style = matlab-editor, language=Matlab, caption=Huey Landing Solution Code]{6DoF Explanation Scripts/Example Problems/P4_2_Script.m}\label{P4-2 Solution Code}

\chapter{Proofs and Derivations}\label{sec:ProofsAndDerivations}
It is often useful, especially for mathematical reasoning, to see more exact derivations. We have included several of the most useful derivations in this appendix.

\section{Buckingham Pi}\label{Buckingham Pi Deriv}
...
\begin{figure}[ht]
    ...
    \label{fig:qr code}
\end{figure}
The rest of the code shown below link to certain sections of this GitHub repository and are reproduced only for convenience. 
