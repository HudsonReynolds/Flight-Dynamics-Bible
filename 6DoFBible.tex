...
From \autoref{thm: buck pi corollary}, we can also write
...
In this final chapter, we discuss the techniques and practices for implementing a 6-DoF in MATLAB. We will also discuss more general practices for large MATLAB code bases and methods to keep these organized. As of Fall 2025, these practices are currently being implemented on our 6-DoF model. A large portion of the content in this chapter comes from what we have seen as practices adopted in industry for systems much more complex and larger than our own.
...
Public and private methods / properties

argument 

\color{black}

Up until now, we have mainly been using a procedural approach to programming. The procedural approach uses multiple interacting functions to run the code. Each function takes in inputs and outputs. Together, these functions work together to run the code and built the final output. In most introductory coding classes, this is the type of programming that is taught.

This is a very good method for writing short segments of code, but for larger databases that need to have greater code reusability, there are better approaches. One such approach that is particularly powerful is \gls{oop}.\endnote{There exists some debate whether \gls{oop} is a superior method of coding. Of course, the true answer is nuanced and depends on the application. Object-Oriented programming, when done poorly, can make code quite unreadable. However, proper implementation of \gls{oop} can be extremely useful. See \cite{brian_will_object-oriented_2016}  for more information.} 
...
\Gls{oop}, like any skill in coding, is best understood with practice. As a starting point, we highly recommend \cite{tearle_object-oriented_nodate}, which gives a guided introduction to \gls{oop} in MATLAB. It may take several repititions and read-throughs to become familiar with the terminology of \gls{oop}.
...
An object is a reuseable part of code that has certain attributes and functions associated within it. An object may be a "rocket", and it has features such as "propellant type", "mass", and "thrust", to name a few. The rocket might also have some functions associated with it, such as "SimulateFlight", which would simulate the rocket flight based on all the given inputs.
...
% talk about precedence and how overloading is achieved

\subsection{Subclasses and Superclasses}
Classes have the feature that they can share portions of their code with other classes. When a class takes it's base code from another class, this is referred to as \textit{inheritance}. The class which is inheriting is the \textit{subclass}, and the class which is being inherited from is the \textit{superclass}.

The construct of the subclass and superclass is use
...
Classes in MATLAB carry lots of information. As a result, documenting them is quite critical for maintaining and improving their functionality. In our 6-\gls{dof} modeling, we require that all class definitions use the following header:
\begin{figure}
    \centering

\begin{lstlisting}[style=Matlab-editor, caption = Class Header]
% Short 2-3 line explanation of the function
    %
    % Required 
    % 
...
\end{lstlisting}
    ...
    \label{fig:Class Header}
\end{figure}

% include information on the script headers / help for 
...
Unit testing works in combination with continuous integration practices and the object-oriented approach to provide the capability to test the functionality of MATLAB code.
...
\begin{enumerate}
\begin{enumerate}
    \item We can derive the \gls{eom} of the system using either the \gls{Lagrangian} or Newtonian methods. The Lagrangian method is detailed in \autoref{sec:energy methods}, so it will not be covered here. The \gls{eom} of the system is derived by considering the force balance of the system:
    $$\sum \vec{F}=m\vec{a}$$
...
\item Parts C and D are shown in the same script:
\newpage
