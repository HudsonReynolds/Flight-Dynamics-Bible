...
\lstset{
    ...
    breaklines=true,                  % Optional: breaks long lines
    showstringspaces=false            % Optional: cleaner string appearance
}
...
For those interested solely in Attitude Dynamics, \autoref{sec:attitude dynamics}, Volume II, and Part I of Volume III (Particularly the Chapter on Lie Theory) are suggested. If the reader's interests are in spacecraft dynamics in general, Part II of Volume III is additionally suggested.
...
\lstinputlisting[style=Matlab-custom, caption = Script Header]{6DoF Explanation Scripts/ScriptHeader.m}
\chapter{Overview}
...
\newpage
\lstinputlisting[style=Matlab-custom,  caption=1 DoF]{6DoF Explanation Scripts/OneDofEuler.m}\label{Script 1}
...
\lstinputlisting[style = Matlab-custom, language=Matlab, caption=Euler Angle Dynamics Example]{6DoF Explanation Scripts/Attitude Dynamics Examples/GravityToBodyEuler.m}\label{Euler Dynamics Listing}
...
\begin{lstlisting}[style=Matlab-custom, caption=RK2 Integrator]
%% RK2 Integrator
...
\begin{lstlisting}[style=Matlab-custom, caption=RK4 Integrator]
%% RK4 Integrator
...
\lstinputlisting[style=Matlab-custom, caption = Rotation Matrix]{6DoF Explanation Scripts/RotationMatrix.m}\label{code:RotationMatrix}
...
\chapter{MATLAB Practices}\label{sec:MATLAB Practices}

In this final chapter, we discuss the techniques and practices for implementing a 6-DoF in MATLAB. We will also discuss more general practices for large MATLAB code bases and methods to keep these organized. As of late May 2025, these practices are currently being implemented on our 6-DoF model.

\section{Object Oriented Programming in MATLAB}

\newacronym{oop}{OOP}{Object Oriented Programming}

Up until now, we have mainly been using a procedural approach to programming. This is a very good method for writing short segments of code, but for larger databases that need to have greater code reusability, there are better approaches. One such approach that is particularly powerful is \gls{oop}.

In \gls{oop}, the basis of our code is objects. These objects hold information of all types, whether that be numeric, string, or time data. The most important aspect of \gls{oop} is that it allows us to make user interaction with a large code base simpler, and also allows us to reuse large parts of the code.

If it has not already become apparent, our model is fairly simple in the grand scheme of things. We are modeling a single rocket and most of the subsystems are simplified. What happens if we want to model a huge rocket system with different systems, sensors, measurements, errors, and so on? In this case, we need a better scheme for organization.
...
\lstinputlisting[style=Matlab-custom, caption = Startup File for Plotting]{startup.m}\label{code:startup}
...
\lstinputlisting[style=Matlab-custom, caption = Lorenz Attractor]{6DoF Explanation Scripts/Lorenz Attractor/LorenzAttractor.m}\label{code:Lorenz}

We also have a script to display the results of this in a nice animation:

\lstinputlisting[style=Matlab-custom, caption =Lorenz Animation]{6DoF Explanation Scripts/Lorenz Attractor/LorenzAnimation.m}\label{code:LorenzAnim}
\subsection{Dzhanibekov Effect Model}
\lstinputlisting[style=Matlab-custom, caption = Dzhanibekov Main]{6DoF Explanation Scripts/Dzhanibekov/DzhanibekovMain.m}\label{code:DzhanMain}

\lstinputlisting[style=Matlab-custom, caption = Dzhanibekov Integrator]{6DoF Explanation Scripts/Dzhanibekov/DzhanibekovIntegrator.m}\label{code:DzhanInt}

\lstinputlisting[style=Matlab-custom, caption = Dzhanibekov Rotation Visualizer]{6DoF Explanation Scripts/Dzhanibekov/RotationsVisualizer.m}\label{code:DzhanRotVis}\


\subsection{Numerical Integration Error Examples}
\lstinputlisting[style=Matlab-custom, caption = Numerical Integration Error Comparison]{6DoF Explanation Scripts/NumericalIntegratorError.m}\label{code:Numerical Error $y'=2t$}

\lstinputlisting[style=Matlab-custom, caption = Numerical Integration of Periodic Function]{6DoF Explanation Scripts/NumericalIntegratorErrorNyquist.m}\label{code:Numerical Error y'=cos(4*pi*t)}
