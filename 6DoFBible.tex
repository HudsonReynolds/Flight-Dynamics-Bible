...
\begin{enumerate}
    ...
    \item Numerically integrate the \glspl{eom} ascertain the position and velocity as a function of time. Graph the solution. How does it compare to the analytical solution?
...
There are two main types of energy mechanics, referred to as Hamiltonian and Lagrangian Mechanics. In this chapter, we will mostly focus on the Lagrangian version of these mechanics.
...
We recall that in Newtonian mechanics, we describe reference frames for an object. We represent the position, velocity, and acceleration of an object in terms of these coordinate systems as a tridimensional vector. 


Next we will show a problem where the rotational invariance proves especially useful in simplifying our derivation:
...
\item  A satellite is in the earth-moon system. Assume that the orbit of the earth and the moon are circular. The weight of the satellite is negligible compared to the weight of the earth and moon.
\begin{enumerate}
    \item Find the EOM of the system governing this interactin
\end{enumerate}
\end{enumerate}
...
\section{Practice Problems}
\begin{enumerate}
    \item Your old friend Jimothy, impressed by your MATLAB skills, has challenged you to model a spinning top. The top is axially symmetric about its $\hat{X}$-axis and has an initial spin rate of 1200 rpm and has an initial angle to the vertical of $20\degree$. The top spins down at a rate governed by an exponential decay of form $\omega(t)=\omega_0e^{\frac{1}{10}\ln(2)t}$ such that the spin rate is halved every 10 seconds. Gravity acts on the top in the negative $\hat{x}$-direction. The top has a mass of 20 grams. Relevant details are given in \ref{fig:Spinning Top}.
...
\begin{enumerate}
    ...
    \item Describe the forces and moments acting on the system and write the \glspl{eom} for the system.
...
\newglossaryentry{buck pi}
{
    ...
    description={In a system with $N$ parameters and $K$ physical dimensions, it is possible to express$P=N-K$ dimensionless groups}
}
...
Applying \gls{buck pi}, we can create 3 $\Pi$ groups from the 6 variables.
...
\end{gather}
$\Pi_1$ is a quantity known as the Reynolds number, the ratio of inertial forces to viscous forces. $\Pi_2$ is the Mach number, the ratio of velocity to the speed of sound of the fluid. $\Pi_3$ is the force coefficient that we have described earlier.

From \ref{thm: buck pi corollary}, we can also write
$$\Pi_3=f\left(Re,M\right)$$
This is a very important condition of the theorem, showing that the force coefficient is a function of the Reynolds number and Mach number (as well as the angle of attack). Thus, whenever we do modeling of aerodynamics, we only need to consider Reynolds number, Mach number, and angle of attack. This will become a cornerstone of how we model aerodynamics in our 6-\gls{dof} modeling.
