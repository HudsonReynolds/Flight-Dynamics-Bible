...
\and
\textsc{Preston Wright} \\ \textit{Editor? IDK i just did the attitude stuff}
}
     ...
     \delta&\mathrm{variation}\\
      ...
      L & \mathrm{Lagrangian}\\
       ...
       k& \mathrm{spring \ constant}\\
        M& \mathrm{number\ of\ degrees\ of\ freedom}\\
...
\end{itemize}
File headers are of the following format:

\lstinputlisting[style=Matlab-editor, caption = Lorenz Attractor]{6DoF Explanation Scripts/Lorenz Attractor/LorenzAttractor.m}
\chapter{Overview}
...
\end{chapquote}
The first step when approaching any physics problem of this type is to consider the Newtonian dynamics of the system. In our case, we define Newtonian dynamics as the translational dynamics of the system as well as the moments acting on the system. This differs from particle dynamics because we are considering a rigid body which can exhibit rotation. The exploration of this rotation will be explored in much more detail in Section \ref{sec:energy methods} and\ref{sec:attitude dynamics}. 
...
In Figure \ref{Figure 1}, a total of three forces are shown, all of which act along the $\hat{y}$-direction. The colinearity of all these forces is what makes this a 1-\gls{dof} model. The three forces shown are the thrust force, the drag force, and the gravity force.
...
$$\color{blue}\boxed{\color{black}\vec{M}=\vec{r}\times \vec{F}_R}$$
...
\color{blue}\boxed{\color{black}\vec{M}^o=^{i}\frac{d\vec{H}^o}{dt}}\\
...
\color{blue}\boxed{\color{black}\begin{array}{ll}
     ...
     & \\M_y=I_y\dot{\omega}_y+\left(I_x-I_z\right)\omega_z\
...
There are two main types of energy mechanics, referred to as Hamiltonian and Lagrangian Mechanics. In this chapter, we will mostly focus on the Lagrangian version of these mechanics. Hamiltonian mechanics are arguably more fundamental, but are beyond the scope of our current discussion.
...
\color{blue}\boxed{\color{black}M=3N-K}\color{black}
\end{gather}
The quantity $M$ is the number of \gls{dof}. However, this is the number of \gls{dof} for the whole system, which is why it can exceed 6 in some cases!
...
An example of these constraints would be the position of the first particle in Cartesian coordinates (giving 3 of the generalized coordinates, $x,y,z$, and the azimuthal and elevation angle $\phi,\theta$. The roll angle does not change the system in any measurable way, hence we only need 5 parameters to define a two particle system.
...
That said, we often choose to use more parameters, particularly in the case of rotations. As we will see in \ref{sec:attitude dynamics}, we often find it more useful to use a non-minimal set of constraints to encode rotations for reasons to avoid singularities in the solution.
...
After the formulation of Lagrangian Mechanics, another formulation was created by Hamilton later on. The Hamiltonian formulation, like the Lagrangian, does not introduce any new concepts, because it is simply an alternate approach to dynamics. However, like the Lagrangian, it makes some types of problems more simple. We present the Hamiltonian here briefly for its use case in \textit{symplectic integrators}.
...
Using relations with the Lagrangian, we can write two first order differential equations for the \gls{eom} with the Hamiltonian.
...
\begin{enumerate}
    ...
    \item The methodology of Newtonian mechanics and Lagrangian mechanics are fundementally different. Newtonian mechanics deals with the influence of external actions (forces), while Lagrangian Mechanics emphasizes the internal state of the system.
    \item 
\end{enumerate}
\section{Further Reading}
...
\begin{enumerate}
    ...
    \item Write a general expression for the amount of time it takes a particle to descent a path using an integral
    \item Use the Euler-Lagrange equation to solve for the form of this curve. Plot the result.
\end{enumerate}
...
To match industry/academia standards, our 6DoF model uses quaternions. The use of hyper-imaginary numbers allows us to avoid losing a degree of freedom at a specific orientation in our rocket, making the simulation more robust. Just as with Euler angles, MATLAB has an extensive suite of functions that make the implementation of quaternions just as easy. 
