...
The essence of the 6-\gls{dof} is to simulate the movements of a system through time. To do so, we need some description of how the system may respond to certain forces. We will spend quite a while discussing how these forces arise and how we may use them to understand the subsequent motion of a vehicle.
...
The last issue that we tackle when different reference frames get involved is the fact that objects are moving around, so it can be hard to express quantities in an inertial frame. We can create various reference frames, but to apply laws of physics we want to use an inertial frame.
...
We can solve this by computing the rotation of one reference frame with respect to the other. This will give us the ability to compute a valid derivative.

We can solve this by splitting the velocity into two components, a translational component and a rotational component. The translational component is simply the velocity of the center of mass, which is the time derivative of the position at the center of mass.

For the rotation, we consider the lever arm from point $O$ to point $A$. When the Frisbee rotates, this lever arm creates some rotation of point $A$ about point $O$. To solve for this rotation, we simply consider the cross product of the position vector with the rotation vector, $\omega$.


$${}^e\frac{d}{dt}\vec{r}={}^a\frac{d}{dt}{\vec{r}}+{}^e\vec{\omega}^a\times\vec{r}$$

\end{example}

In general, we can use the \gls{bke} to express frame translations. We can perform analysis using multiple intermediate frames to build a sequence of 
...
\end{figure}
The discussion of these systems in the volume is curtailed because it is not directly applicable to the work on our specific 6-\gls{dof} model. However, the fundamental implications of energy methods are important, but that discussion is saved for Volume III.

A background on the calculus of variations, which provides a derivation for Lagrangian mechanics, is also presented in Volume III. We present here only the formulaic approach and its used.

\section{Hamilton's Principle}

Before we can get to Hamilton's principle, we need to understand a new type of calculus, one which is the calculus of whole functions. We call this type of calculus the calculus of variations. The calculus of variations underpins the ideas of \gls{Lagrangian} and Hamiltonian mechanics, so it is important to understand the principles behind it before moving on to the actual mechanics. In analogy to how single variable calculus is fundamental to Newtonian mechanics, the calculus of variations is fundamental to energy mechanics.
...
\end{equation}
The \gls{functional}, $J$, is a quantity which maps a function onto the real number line. This means that it takes the function as an input and outputs a single real number (that is, a function of a function). We are attempting to find an input function, $y(x)$, that yields an extrema of $J$. Our goal is to find criteria for $y(x)$ that make $J$ a maximum or a minimum.

For now, we present the way to do this without derivation. The fundamental equation of the calculus of variations is called the \gls{Euler Equation}:
...
\end{figure}
The code that runs this example is shown in \autoref{code:Numerical Error $y'=2t$} in the Appendix. This code is a good example for understanding the syntax for using the integration code snippets in Listing \autoref{code:rk2} and \autoref{code:rk4}.
...
The full code can be found at: \cite{reynolds_hudsonreynoldsflight-dynamics-bible_nodate} or in the appendix in \autoref{sec:6DoF Code}.
...
Sub-component validation may look like the Dzhanibekov modeling in \autoref{sec:Dzhan}, where we test the attitude dynamics.

Full model validation is usually more complex. One such example is our validation of our model against the V-2 rocket launch on March 7, 1949. This comparison is shown in \autoref{fig:V2 sim}
...
The basis of the \gls{oop} approach are, as the name suggests, objects. Objects hold much more information than standard variables. Typically, a variable contains only one piece of information, such as a string, an integer, or a floating point number. An object, on the other hand, can contain as much information as we want it to! 
...
Objects are inherently related to what are called \textit{classes}. A class defines the attributes and the functions that are associated with an object. The attributes associated with an object are called properties and the associated functions are called methods.
...
This chapter contains lengthier segments of example code from either the 6-\gls{dof} itself or a proof/example relating to it. Since the code is cumbersome to show in full, we have opted to link to the GitHub repository instead.
...
\section{Numerical Integration Error Examples}
