...
\begin{equation}
    bq=b(q_1+q_2i+q_3j+q_4k)=bq_1+bq_2i+bq_3j+bq_4k
\end{equation}
...
We can additionally show that \gls{quaternion} multiplication is both associative and distributive as follows:
$$q\otimes(q'\otimes q'')=(q\otimes q')\otimes q''$$
$$q\otimes(q'+q'')=q\otimes q'+q\otimes q''$$
It is important to note that while \gls{quaternion} multiplication is both associative and distributive, it is NOT commutative. We can, however, use a new operator to describe such an operation.
\subsubsection{Hamiltonian Product}
While widely unused in attitude determination and dynamics for cases of rotations, the Hamiltonian product has a very simple relation to \gls{quaternion} multiplication - especially in the matrix form. Before defining the product of the Hamiltonian product, we can first describe the operator by the circled dot, or $q\odot q'$. Having this, the outcome of the Hamiltonian product is as follows:
\begin{equation}\label{eq:QuatHamiltonProdMatrix}
    \vec{q}\odot\vec{q}'=\begin{bmatrix}
        q_1q_1'-(\vec{q}_{2:4}\cdot \vec{q}_{2:4}')\\
        q_1\vec{q}_{2:4}'+q_1'\vec{q}_{2:4}-(\vec{q}_{2:4}\times\vec{q}_{2:4}')
    \end{bmatrix}
\end{equation}

It is important to note that we can explicitly describe the relation between \gls{quaternion} multiplication and the Hamiltonian product with the following:
$$q\otimes q'=q'\odot q$$

A quick note about the Hamiltonian Product, it shares properties with \gls{quaternion} multiplication. That means it is both associative and distributive; however, it isn't commutative.

\subsubsection{Identity Quaternion}

The identity \gls{quaternion} can be described similar to an identity matrix. Namely it doesn't alter any vector (or in this case any quaternion) it is multiplied with. Logically, this results in the identity \gls{quaternion} having a real part ($q_1$) of one and imaginary part ($q_{2:4}$) of zero like so:
\begin{equation}\label{eq:IdentityQuat}
    I_q=1+0i+0j+0k=\begin{bmatrix}
        1\\\vec{0}_{3:1}
    \end{bmatrix}
\end{equation}
...
\end{figure}
This is so important to the foundations of versors and rotations encoded by quaternions, that the versor itself is expressed using these values. Using this \gls{Euler axis} rotation description of a rotation, we can define the \gls{quaternion} simply with respect to this rotation using the methods described for 2-dimensional complex numbers in section \ref{sec:2DImaginary}: 
...
\begin{figure} [ht]
    \centering
    \includegraphics[width=\linewidth]{Numerical Integrator Nyquist 1.png}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{Numerical Integrator Nyquist 2.png}
    
    \caption{Numerical Integration of Periodic Function with varying}
    \label{fig:Nyquist}
\end{figure}
...
\Gls{stiff} systems make numerical integration challenging not only because of increased simulation time, but also because of aforementioned \gls{trunc error}. In a 6-\gls{dof} model such as ours, this is important if vibrational modeling is considered. It is best practice to try and decouple the simulation of high frequency vibrations from the flight dynamics of the system. Sometimes this may be unavoidable (such as systems with large amounts of wing flexure where aerodynamics and elasticity / vibrations are heavily couples), but should be attempted whenever possible. This problem is very complicated and is the subject of much ongoing research! More information about such dynamics can be found in \cite{shyy_recent_2010}.
\section{Other Integration Schemes}
We previosly discuss the concept of \gls{stiff} systems in \ref{sampling and stiffness}. For systems that have moderate stiffness, we can use "ode23" instead of the more typical "ode45" in MATLAB.
...
The moment arm of the vehicle is equivalent to the distance from the center of mass to the center of pressure. Assuming that both lie along the longitudinal axis, we can express this moment as:
\begin{equation}
    M^c
\end{equation}
...
\lstinputlisting[style=Matlab-editor, caption = Numerical Integration Error Comparison]{NumericalIntegratorError.m}\label{code:Numerical Error $y'=2t$}
