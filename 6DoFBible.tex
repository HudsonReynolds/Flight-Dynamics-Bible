...
Now that we’ve found an approximation for , we can extend this to find approximations for $y_2$, $y_3$, and so on. This formula is generally extensible as:
...
As can be seen from the figure, large timesteps quickly lead to error in the solution. For a strictly convex curve, our Euler approximation will always give us an under approximation of the solution. In some cases, however, our solution may diverge so greatly that it becomes unstable and values tend toward infinity.\footnote{This is especially true for periodic functions where we sample below the Nyquist limit}
...
\lstinputlisting[language=Matlab, caption = Lorenz Attractor]{LorenzAttractor.m}\label{code:Lorenz}

We also have a script to display the results of this in a nice animation:

\lstinputlisting[language=Matlab, caption =Lorenz Animation]{LorenzAnimation.m}\label{code:LorenzAnim}
