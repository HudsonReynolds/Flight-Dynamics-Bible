...
The end of each section also provides several practice problems to ensure the key ideas of each topic are understood by the reader. These problems are expected to be difficult. For readers new to this content, it is expected that each problem may take several hours and some consultation with other sources to complete. However, we have attempted to make these problems challenging while not being out of reach. These problems are designed to make the reader struggle and push their knowledge. Generally, each subsequent problem is more difficult than the last.
...
\begin{enumerate}
    ...
    \item Describe the forces and moments acting on the system and write the \glspl{eom} for the system (You may find the Lagrangian approach simpler!)
...
%solve the pi functions, introduce AoA
...
\subsection{Attitude Dynamics}
These moments are used to find angular acceleration using the Euler rigid body equations described in \eqref{eq: moment eq 2}. 
...
\begin{equation}
    ...
    \end{bmatrix}\vec{q}
\end{equation}
So, taking all of these together, we have the derivative of our state needed for numerical integration.
...
\lstinputlisting[style=Matlab-editor, caption = Numerical Integration Error Comparison]{6DoF Explanation Scripts/6DoF/MainRK4.m}\label{code:mainRK4}
