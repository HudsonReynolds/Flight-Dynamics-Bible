...
A general overview of the scope of this document is given in the preface.  Here, we will go into more detail about the layout of each section. Our goal is to present this document in a similar manner to a textbook while still remaining approachable.
...
Additionally, a  'Further Reading and Notes' is included which the curious reader may benefit from. Many topics are covered in this document; each of which deserve their own book-length discussion. Because we have neither the desire nor capability to write such a long document, we have linked these beneficial books whenever possible. It is not necessary to read any / all of these sources to comprehend the content here, but they may prove useful for deeper research into topic areas that interest the reader.

The end of some sections also provides several practice problems to ensure the key ideas of each topic are understood by the reader. These problems are expected to be difficult. For readers new to this content, it is expected that each problem may take several hours and some consultation with other sources to complete. However, we have attempted to make these problems challenging while not being out of reach. These problems are designed to make the reader struggle and push their knowledge. Generally, each subsequent problem is more challenging than the last.
...
To begin our exploration of Newtonian dynamics, we will return to the example of the 1-\gls{dof} which we used earlier. Our goal with this is to use a simple example that you are familiar with from high school physics and bring it up to speed with the terminology and techniques we will use for more complex analysis.
...
The notation here is important to understand. In future AAE classes at Purdue, this is likely the notation that you will see. Terms with the subscript $\infty$ denote \gls{freestream} quantities, those that are freely flowing far away from the body of interest. Also of note is the unit step function, which is a function which has value zero until reaching time $t_b$, where it thereafter equals one. The quantity  $\rho$ refers to the density of the fluid, and $S$ to the \gls{ref area}. The \gls{drag coeff} is $C_D$, a term which encapsulates the very complex nature of the fluid flow around an object. Calculating the value of $C_D$ will be explored more deeply in later sections of this document. For now, we may assume a constant value.
...
Expressed in vector notation, we write Newton's 2nd Law as $\vec{F}=m{}^i\ddot{\vec{r}}^{op}$. Because ${}^i\ddot{\vec{r}}^{op}$ lies strictly in the $\hat{y}$ direction, it can be easily deduced that ${}^i\ddot{\vec{r}}^{op}=\ddot{y}$. 
...
We follow the same process for the integration of acceleration to find the velocity. Of note, we will use the previous value of $v$ in the computation of the new acceleration. Note that a more formal definition of Euler’s method is given in section \ref{sec:euler integration}.
...
\begin{gather}
    ...
    \end{bmatrix}
\end{gather}
This is done not only for the sake of compactness, but also to facilitate the use of matrix operations for translation between reference frames. This concept is further explored in section \ref{sec:attitude dynamics}. For this reason, expressing these elements in vector form will become crucial as we move forward. In MATLAB, we express a tri-dimensional column vector as:
...
For the sake of completeness, we should also note that the force of drag lies opposite the direction of $V_{\infty}$. This is much simpler to calculate, and looks like:
...
\section{Key Ideas}
\begin{enumerate}
    \item Newton's 2nd Law can be used to create 2nd order differential equations that describe particle motion
    \item An order $n$ differential equations can be reduced into $n$ first order differential equations
    ...
    \item We can describe the properties of a bodies rotation in an analogous way to Newton's 2nd Law.
...
You can consider the door hinge as a mass-spring system constrained to motion along the $\hat{x}$-direction. The spring has an equilibrium length of $L_0$ and a spring constant of $k=5$ N/m with a mass of 1 kg. At time $t_0$, an instantaneous impulse of 10 $\mathrm{N}\cdot \mathrm{s}$ is applied to the hinge in the $-\hat{x}$-direction by a misguided ping-pong ball hitting the door. This is shown diagrammatically in \ref{fig:mass spring}. Assume that the system is at rest in the equilibrium position at $t_0$.
\begin{figure}[ht]
    \centering
...
\begin{enumerate}
    ...
    \item Numerically integrate the \glspl{eom} ascertain the position and velocity as a function of time. Graph the solution. How does it compare to the analytical solution? You may want to refer to \ref{sec:numerical integration schemes} for other numerical integration methods.
    \item Consider the system with some frictional force that is proportional to the velocity, with a proportionality constant of $c=1\mathrm{\frac{kg}{s}}$. This frictional force opposes the motion of the system. Repeat steps a through c with this new system.
\end{enumerate}
...
\end{figure}
\begin{enumerate}
    \item Assuming all 3-\gls{dof}, derive the \gls{eom} for the satellite motion assuming that gravity is the only force acting on the satellite and the earth is a point mass.
...
\end{enumerate}
\end{enumerate}
\chapter{Energy Methods of Mechanics}
\begin{chapquote}{Joseph Fourier}
    ''Mathematics compares the most diverse phenomena and discovers the secret analogies that unite them.''
\end{chapquote}

Energy methods of mechanics employ the concepts of the energy of a system to solve a problem in mechanics. This is in contrast to the Newtonian method, which considers the forces of a system. The good news is that both will result in the same answer. However, having both in your set of tools as an engineer can prove especially useful.

To grasp energy methods, it is best to have an intuition for Newtonian dynamics first. A thorough reading of \ref{sec: Newtonian Dynamics} and other sources will help prepare the reader for this section.

There are two main types of energy mechanics, referred to as Hamiltonian and Lagrangian Mechanics. In this chapter, we will mostly focus on the Lagrangian version of these mechanics. Hamiltonian mechanics are arguably more fundemental, but are beyond the scope of our current discussion.

Although the Hamiltonian and Lagrangian are not directly used in the case of our 6-\gls{dof}, they are fundamental to other areas of mechanics and make derivations far simpler in many cases. The Hamiltonian and Lagrangian descriptions of mechanics are more often taught to physics students rather than engineers, but we feel that they deserve inclusion among the topics presented here. When we discuss numerical integration, we can use \textit{symplectic} integrators when our problem has a Hamiltonian description. This will be further discussed in \ref{sec:other numerical ints}. You may find that certain \gls{eom} derivations, such as the spinning top and problems in orbital mechanics are more easily accomplished with energy methods.

Another reason to motivate these systems is that these energy methods are often rotationally invariant because energy methods can employ a generalized coordinate system. We will see this later in an example of a pendulum.

A brief overview of the differences between the two schemes of \gls{eom} derivation is shown in \ref{fig:newt v lang}
...
%Flowchart: Decision [id:dp7233285773485726] 
\draw  [fill={rgb, 255:red, 207; green, 207; blue, 207 }  ,fill opacity=1 ] (300,110) -- (350,150) -- (300,190) -- (250,150) -- cycle ;
%Straight Lines [id:da23285688596679088] 
\draw    (210,150) -- (248,150) ;
...
%Flowchart: Alternative Process [id:dp302743495564027] 
\draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=0 ] (210,40.5) .. controls (210,34.7) and (214.7,30) .. (220.5,30) -- (509.5,30) .. controls (515.3,30) and (520,34.7) .. (520,40.5) -- (520,79.5) .. controls (520,85.3) and (515.3,90) .. (509.5,90) -- (220.5,90) .. controls (214.7,90) and (210,85.3) .. (210,79.5) -- cycle ;
%Straight Lines [id:da32370945071471136] 
\draw    (300,110) -- (300,92) ;
\draw [shift={(300,90)}, rotate = 90] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da5150501682503866] 
\draw    (300,190) -- (300,208) ;
\draw [shift={(300,210)}, rotate = 270] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Flowchart: Alternative Process [id:dp4701889184197712] 
\draw   (120,137) .. controls (120,133.13) and (123.13,130) .. (127,130) -- (203,130) .. controls (206.87,130) and (210,133.13) .. (210,137) -- (210,163) .. controls (210,166.87) and (206.87,170) .. (203,170) -- (127,170) .. controls (123.13,170) and (120,166.87) .. (120,163) -- cycle ;
%Flowchart: Alternative Process [id:dp9991566127280761] 
\draw   (220,47) .. controls (220,43.13) and (223.13,40) .. (227,40) -- (273,40) .. controls (276.87,40) and (280,43.13) .. (280,47) -- (280,73) .. controls (280,76.87) and (276.87,80) .. (273,80) -- (227,80) .. controls (223.13,80) and (220,76.87) .. (220,73) -- cycle ;
%Flowchart: Alternative Process [id:dp3781527195949975] 
\draw   (300,47) .. controls (300,43.13) and (303.13,40) .. (307,40) -- (383,40) .. controls (386.87,40) and (390,43.13) .. (390,47) -- (390,73) .. controls (390,76.87) and (386.87,80) .. (383,80) -- (307,80) .. controls (303.13,80) and (300,76.87) .. (300,73) -- cycle ;
%Flowchart: Alternative Process [id:dp44246911339647754] 
\draw   (220,227) .. controls (220,223.13) and (223.13,220) .. (227,220) -- (293,220) .. controls (296.87,220) and (300,223.13) .. (300,227) -- (300,253) .. controls (300,256.87) and (296.87,260) .. (293,260) -- (227,260) .. controls (223.13,260) and (220,256.87) .. (220,253) -- cycle ;
%Flowchart: Alternative Process [id:dp3723988845908619] 
\draw   (320,227) .. controls (320,223.13) and (323.13,220) .. (327,220) -- (413,220) .. controls (416.87,220) and (420,223.13) .. (420,227) -- (420,253) .. controls (420,256.87) and (416.87,260) .. (413,260) -- (327,260) .. controls (323.13,260) and (320,256.87) .. (320,253) -- cycle ;
%Flowchart: Alternative Process [id:dp44680017489514834] 
\draw   (410,47) .. controls (410,43.13) and (413.13,40) .. (417,40) -- (503,40) .. controls (506.87,40) and (510,43.13) .. (510,47) -- (510,73) .. controls (510,76.87) and (506.87,80) .. (503,80) -- (417,80) .. controls (413.13,80) and (410,76.87) .. (410,73) -- cycle ;
%Flowchart: Alternative Process [id:dp3986497092067455] 
\draw   (440,227) .. controls (440,223.13) and (443.13,220) .. (447,220) -- (503,220) .. controls (506.87,220) and (510,223.13) .. (510,227) -- (510,253) .. controls (510,256.87) and (506.87,260) .. (503,260) -- (447,260) .. controls (443.13,260) and (440,256.87) .. (440,253) -- cycle ;
%Straight Lines [id:da07392727672316557] 
\draw    (300,240) -- (318,240) ;
\draw [shift={(320,240)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da24064295385025491] 
\draw    (420,240) -- (438,240) ;
\draw [shift={(440,240)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
%Straight Lines [id:da6713215946680514] 
\draw    (280,60) -- (298,60) ;
...
%Straight Lines [id:da463019374864058] 
\draw    (390,60) -- (408,60) ;
...
%Flowchart: Alternative Process [id:dp13808176557797036] 
\draw  [fill={rgb, 255:red, 74; green, 144; blue, 226 }  ,fill opacity=0 ] (210,220.5) .. controls (210,214.7) and (214.7,210) .. (220.5,210) -- (509.5,210) .. controls (515.3,210) and (520,214.7) .. (520,220.5) -- (520,259.5) .. controls (520,265.3) and (515.3,270) .. (509.5,270) -- (220.5,270) .. controls (214.7,270) and (210,265.3) .. (210,259.5) -- cycle ;

% Text Node
\draw (165,150) node  [font=\normalsize] [align=left] {Governing\\Mechanics};
% Text Node
\draw (300,150) node  [font=\normalsize] [align=left] {\begin{minipage}[lt]{36.77pt}\setlength\topsep{0pt}
\begin{center}
EOM\\Method
\end{center}
...
\end{minipage}};
% Text Node
\draw (301,28) node [anchor=south west] [inner sep=0.75pt]  [font=\normalsize] [align=left] {\begin{minipage}[lt]{50.94pt}\setlength\topsep{0pt}
\begin{center}
Newtonian
\end{center}
...
\end{minipage}};
% Text Node
\draw (301,272) node [anchor=north west][inner sep=0.75pt]  [font=\normalsize] [align=left] {\begin{minipage}[lt]{53.81pt}\setlength\topsep{0pt}
\begin{center}
Lagrangian
\end{center}
...
The fundamental idea of the calculus of variations is simple. We want to find the maximums and minimums of some quantity in our problem. Why? In physics, we often see that quantities are minimized or maximized for some reason. For example, a soap surface will minimize its surface area, or a beam of photons will follow a \textit{geodesic} of spacetime. Light will also refract through surfaces to minimize time as seen in Snell's Law. 
...
Before we get to Hamilton's formulation, we need to understand what the calculus of variations is trying to do. To find a minima means that naturally, we should look for the extrema of some quantity. We call this quantity a \textit{functional}, which we define as:
...
Using this definition of the neighborhood function, we can rewrite \eqref{eq:functional} as
...
\end{equation}
The $\varepsilon$ terms drop out from the RHS from the evaluation of $\varepsilon=0$. Applying the \textit{Leibniz Integral Rule}, the partial derivative can be moved inside the RHS integral:
\begin{equation}
...
This gives us the fundamental equation of the calculus of variations, called \textit{Euler's Equation}:
...
The quantity $T-U$, the kinetic energy minus the potential energy, is a quantity called the \textit{Lagrangian}, denoted $L$. 
\begin{equation}
    L\equiv T-U
\end{equation}

The time integral of the Lagrangian is the \textit{action}, denoted $S$. This is often why Hamilton's Principle is also called the \textit{Principle of Least Action}. 
...
$$m\ddot{x}+kx=0$$
So, in essence, we are using the Lagrangian \textit{a posteriori } to match this solution.
...
$$m\ddot{x}+kx=0$$
In this problem, we used a simple case where the dynamics were constrained along a single axis of motion. Before we move to a move complex problem, we will introduce the method by which Lagrangian mechanics deals with these reference frames.
...
The acceleration of the particles, when Newton's Laws are applied, are a function of the $q_j$ as well as their derivatives, $\dot{q}_j$. So, the evolution of the system exists in a $2n$-dimensional state space. However, we can use our previously mentioned Lagrangian to circumvent the need for Newton's Law and the \gls{bke} in our derivation of the \gls{eom}.

\subsection{Constraints}\label{sec:constaints}
In the previous section, we very vaguely stated the need for $n$ generalized coordinates. To actually find the number
...
$$f_k\left(\vec{r}^{\ op_1},\vec{r}^{\ op_2}\cdots \vec{r}^{\ op_N}\right)$$
The number of holonomic constraints, $K$, will change the number of generalized coordinates as:
\begin{equation}\label{eq:dof}
    M=3N-K
\end{equation}
The quantity $M$ is the number of \glspl{dof}. However, this is the number of \glspl{dof} for the whole system, which is why it can exceed 6!
...
\begin{gather}
    ...
    q_2-g(x)=0
\end{gather}
With these two constraints, we have reduced the number of degrees of freedom, $M$ to 1.
...
In general, these generalized coordinates do not even need to have units of length! We can select any parameters that are most convenient for defining the state.

\subsubsection{Rigid Body Constraints}
...
So far, we have only discussed holonomic constraints, those that are related to position or on the generalized parameters, and possibly have some relation in time. When a system is a \textit{holonomic system}, that is, purely defined by holonomic constraints, it is always possible to find a set of $M$ generalized parameters to describe the particle motion. In other words, the minimum number of parameters is equivalent to the number of degrees of freedom in the system.

That said, we often choose to use more parameters, particularly in the case of rotations. As we will see in \ref{sec:attitude dynamics}, we often find it more useful to use a non-minimal set of constraints to encode rotations for reasons that are explained in that section.
...
The other class of constraints are those constraints that are not merely a function of the generalized coordinates. These constraints, called \textit{non-holonomic constraints}, define the set of all constraints that depend on more than just the position of particles.

The most common among these, and where we will focus our attention here, is constraints on the time derivative of the generalized coordinates. These are called \textit{generalized velocities}, denoted $\dot{q}_j$. 
...
\begin{equation}\label{eq:Euler-Lagrange General}
\frac{\partial L}{\partial q_j}-\frac{d}{dt}\frac{\partial L}{\partial \dot{q}_j}=0,\ \ \ j=1,2,\cdots M
\end{equation}
...
$$T\hat{a}_r=mg\cos\theta \hat{a}_r$$
...
$$\ddot{\theta}+\frac{g}{L}\sin\theta=0$$
We notice that with the Lagrangian approach, we did not need to have an understanding of the vector directions, frame translations, or \gls{bke} to find our \gls{eom}. This, in addition to the flexibility of the generalized coordinates, is the real power of this approach.

\section{Generalized Forces}

\section{Lagrangian Derivation of Rigid Body Dynamics}
THIS SECTION SHOULD BE MOVED TO THE ATTITUDE DYNAMICS VOLUME

In section \ref{Euler Rotation Equations}, we explored the Newtonian method of deriving the Euler rotation equations using the \gls{bke}. Here, we present an alternative derivation that utilizes the Lagrangian approach and is ultimately a more fundemental and simple derivation.
...
\end{equation}
We can select our generalized coordinates as the three Euler angles, $\psi$, $\theta$, and $\phi$.
...
\section{Key Ideas}
\begin{enumerate}
    \item We can find the \gls{eom} of a system using either the Newtonian approach or the Lagrangian approach, and it is often easier to find an \gls{eom} using the Lagrangian for a conservative system.
    \item The Lagrangian is scalar, so it is invariant to coordinate transforms. This means we can choose any variables we want for our generalized coordinates when performing derivations.
\end{enumerate}
\section{Further Reading}

\section{Practice Problems}
\begin{enumerate}
    \item Tony the Elephant keeps finding bugs on the peanuts that the zookeepers are feeding to him. Tony heard that you have been learning about generalized coordinates. He wants a way to describe the position of the bugs on his peanuts using a minimal set of coordinates.

Find a suitable set of generalized coordinates to describe this system. Write out any equations of constraints of the system. (Hint: A peanut is topologically similar to a sphere)
    \item You are a mathematician in the year 1696. Bernoulli proposes a problem to you to find a curve from a point $(x_0,y_0)$ to $\left(x_f,y_f\right)$ under the influence of only gravity that takes the least amount of time.
%    \item brachistochrone

%this one is probably too hard below:
\item  A satellite is in the earth-moon system. Assume that the orbit of the earth and the moon are circular. The weight of the satellite is negligible compared to the weight of the earth and moon. (CR3BP)
\begin{enumerate}
    \item Find the EOM of the system governing this interaction
    \item 
\end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item  write the \glspl{eom} for the system. If you choose the Newtonian approach, write the forces and moments of the system. If you choose the Lagrangian Approach, write the energy equations and constraint equations.
...
\end{enumerate}
\item After your stint in modelling the Explorer 1 Satellite, NASA has asked you to come back to model a new satellite. NASA wants the satellite to be nadir-pointing at all times, and has decided to use gravity gradient stabilization to do so. 
\end{enumerate}
...
\cite{noauthor_httpsengineeringpurdueeduwassgrenteachingme30800notesandreadingdimensionalanalysis_buckinghampi_methodofrepeatingvariables_readingpdf_nodate}
...
\subsection{6-DoF Code}
\lstinputlisting[style=Matlab-editor, caption = 6-DoF Main Script]{6DoF Explanation Scripts/6DoF/MainRK4.m}\label{code:mainRK4}

\lstinputlisting[style=Matlab-editor, caption = 6-DoF Integrator]{6DoF Explanation Scripts/6DoF/RK4Integrator.m}\label{code:6DoF Integrator}
